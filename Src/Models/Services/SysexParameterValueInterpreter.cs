...
public class SysexParameterValueInterpreter
{
        ...
        out string stringValue)
    {
        ...
        if (parspec.Type == Integra7ParameterSpec.SpecType.NUMERIC)
        {
            ...
            if (parspec.IMin != parspec.OMin || parspec.IMax != parspec.OMax || parspec.IMin2 != parspec.OMin2 ||
                parspec.IMax2 != parspec.OMax2)
            {
        ...
        else if (parspec.Type == Integra7ParameterSpec.SpecType.DISCRETE)
        {
            ...
            {
                Log.Error(
                    $"Discrete value {val} has not known value for parameter {parspec.Path}. Choosing something else instead.");
