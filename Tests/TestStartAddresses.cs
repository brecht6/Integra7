...
public class StartAddressesTests
{
    ...
    public void TestSynthTonePartials()
    {
        ...
        byte[] data4 = _a.Lookup("Offset/SuperNATURAL Synth Tone Partial 1").Address;
        Assert.That(data4, Is.EquivalentTo((byte[])[0x00, 0x20, 0x00]));
        byte[] data5 = _a.Lookup("Offset/SuperNATURAL Synth Tone Partial 3").Address;
        Assert.That(data5, Is.EquivalentTo((byte[])[0x00, 0x22, 0x00]));
        Assert.That(_a.Exists("Offset/SuperNATURAL Synth Tone Partial 4"), Is.EqualTo(false));
    }
    ...
    {
        byte[] data = _a.Lookup("Offset/SuperNATURAL Drum Kit Partial 27").Address;
        Assert.That(data, Is.EquivalentTo((byte[])[0x00, 0x10, 0x00]));
        byte[] data2 = _a.Lookup("Offset/SuperNATURAL Drum Kit Partial 88").Address;
        Assert.That(data2, Is.EquivalentTo((byte[])[0x00, 0x4d, 0x00]));
        Assert.That(_a.Exists("Offset/SuperNATURAL Drum Kit Partial 89"), Is.EqualTo(false));
    }
}