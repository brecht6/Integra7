...
public class FullyQualifiedParameterRange
{
    ...
    public FullyQualifiedParameterRange(string start, string offset, string offset2, Integra7ParameterSpec firstPar, Integra7ParameterSpec lastPar)
    {
        _start = start;
        _offset = offset;
        _offset2 = offset2;
    ...
    public void Initialize(List<FullyQualifiedParameter> parameters)
    {
        ...
        {
            _range.Add(new FullyQualifiedParameter(_start, _offset, _offset2, parameters[i].ParSpec, parameters[i].RawNumericValue, parameters[i].StringValue));
        }
    }
    ...
    public void WriteToIntegra(IIntegra7Api integra7Api, Integra7StartAddresses startAddresses, Integra7Parameters parameters)
    {
        ...
        byte[] totalAddr = ByteUtils.AddressWithOffset(startAddr, offsetAddr, offset2Addr, firstParameterAddr);
        byte[] data = [];
    ...
    public void RetrieveFromIntegra(IIntegra7Api integra7Api, Integra7StartAddresses startAddresses, Integra7Parameters parameters)
    {
        ...
        {
            Log.Error("Unfortunately, no reply received after making a sysex data request. This may indicate a bug in the program, e.g. requesting parameters for a PCM synth tone if no PCM synth patch is active or having multiple instances of the application running at the same time.");
        }
    }
